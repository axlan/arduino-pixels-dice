...
// This is directly memory mapped to the data sent over BLE. Packing it avoids
// the compiler adding implicit padding.
#pragma pack(push, 1)
...
// Limit the buffered events if they aren't polled in time
static constexpr size_t MAX_EVENT_QUEUE_SIZE = 100;

// Control BLE scanning behavior
...
// Queues of events from Die
static RollUpdates roll_updates_;
static BatteryUpdates battery_updates_;

// This is the main state for the library.
// It relies on the behavior of maps, that references remain valid unless an
// element is deleted. Iterators need to be protected by mutexes, but accessing
// values shouldn't need to be.
static std::unordered_map<PixelsDieID, DieState> die_map_;

// Mutex to sync event queues
...
void PixelAdvertiseCallbacks::onResult(BLEAdvertisedDevice advertisedDevice) {
  ...
  for (size_t i = 0; i < advertisedDevice.getServiceUUIDCount(); i++) {
    ...
    // Does the device have the service associated with the Pixels Dice.
    ...
    // The semaphore should never block for more then a few cycles, but there's
    // not much need to tune this.
    ...
    // This would deadlock. Connections need to be done outside of the callback.
    // if (auto_connect_) {
    //   ConnectDie(pixelId);
    // }
  }
...
/**
 * Callback from connected dice sending battery or roll data.
 */
...
/**
 * Try to establish connections to dice that have been found and have been
 * flagged for connection. Disconnect from dice that are flagged for disconnect.
 */
static void UpdateDieConnections() {
